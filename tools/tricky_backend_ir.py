...
MNEMONICS = {
    ...
    0x31: "stw", 0x32: "stwu", 0x33: "stwx", 0x3C: "add", 0x3D: "addc", 0x3E: "adde", 0x3F: "addi",
    0x42: "addis", 0x44: "addze", 0x45: "divw", 0x46: "divwu", 0x47: "mulhw", 0x48: "mulhwu", 0x49: "mulli", 0x4A: "mullw", 0x4B: "neg",
    0x4C: "subf", 0x4D: "subfc", 0x4E: "subfe", 0x4F: "subfic", 0x52: "cmpwi", 0x53: "cmpw",
    0x54: "cmplwi", 0x55: "cmplw", 0x56: "andi.", 0x58: "ori", 0x59: "oris", 0x5A: "xori",
    0x5B: "xoris", 0x5C: "and", 0x5D: "or", 0x5E: "xor", 0x62: "andc", 0x64: "extsb",
    0x65: "extsh", 0x66: "cntlzw", 0x67: "clrlwi clrrwi rlwinm rotlwi slwi srwi",
    ...
    0xB8: "fcmpu", 0xB9: "fcmpo", 0x193: "psq_l", 0x195: "psq_lx", 0x197: "psq_st", 0x199: "psq_stx",
}
...
def validate_alignment(snapshot, assembly, code):
    ...
    for index, (instruction, asm) in enumerate(zip(instructions, assembly)):
        ...
        if op not in (0x01, 0x13):  # Calls also carry implicit argument/clobber operands.
            ...
            if "(0)" in operand_text and registers and registers[-1] == ("r", "0"):
                registers.pop()
            # Branch addresses such as f0 or f150 are hex, not FPR operands.
            # Symbol annotations can also contain register-shaped names.
            printed = [] if op in (0x00, 0x05, 0x08, 0x0B, 0x11, 0x12) else re.findall(
                r"\b([rf])(\d+)\b", operand_text)
        ...
        if op in (0x89, 0x8A):
            ...
            if len(args) != 3 or any(
                    ...
                    for a in args):
                raise ValueError("invalid division operands")
            dest, dividend, divisor = args
            expected = ((31 << 26) | (dest["number"] << 21) | (dividend["number"] << 16)
                        | (divisor["number"] << 11) | ((491 if op == 0x45 else 459) << 1))
        elif op == 0x56:
            if len(args) != 4:
            ...
            if len(args) != 5:
                raise ValueError("invalid rotate-and-mask operands")
            dest, source, shift, mask_begin, mask_end = args
            if (any(a["kind"] != 0 or a["register_class"] != 4 or not 0 <= a["number"] < 32
                    for a in (dest, source))
                    or any(a["kind"] != 2 or not 0 <= a["value"] < 32
                           for a in (shift, mask_begin, mask_end))):
                raise ValueError("invalid rotate-and-mask operands")
            expected = ((21 << 26) | (source["number"] << 21) | (dest["number"] << 16)
                        | (shift["value"] << 11) | (mask_begin["value"] << 6) | (mask_end["value"] << 1))
