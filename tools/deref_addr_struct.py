#!/usr/bin/env python3
"""Generic struct ADDRESS-OF -> &member converter, gated by .o byte-identity.
...
def main():
    ...
    for var in varnames:
        def arepl(m, var=var):
            ...
            base_w = {'F32':4,'S32':4,'U32':4,'PTR':4,'S16':2,'U16':2,'S8':1,'U8':1}.get(fcls)
            tyw = 4 if ty.endswith('*') or ty in ('int','uint','u32','s32','f32','float','void') else (2 if ty in ('u16','s16') else 1)
            if ty.rstrip(' *') in (fty.rstrip('*'),) and base_w == tyw:
                stats['addr'] += 1
                return '&%s' % mem
            stats['addr_cast'] += 1
            return '(%s *)&%s' % (ty, mem)
        src = re.sub(
            ...
            r'\(\s*(?:char|u8|s8)\s*\*\)\s*%s\s*\+\s*'
            r'(0x[0-9a-fA-F]+|\d+)\s*\)' % re.escape(var), arepl, src)
