/*
 * zlbDecompress -- retired C reconstruction (ProDG 3.5, 58.585033%).
 *
 * HISTORICAL RECONSTRUCTION: the live source is src/main/zlb.s, an assembled
 * unit that reproduces the retail bytes. This copy is the last ProDG/GCC C
 * draft and stays as a behaviour reference: the bit cursor, the overlapping
 * stored-block length reads and the Huffman table construction are documented
 * in C here. It is not part of the build. See zlb_provenance.md.
 *
