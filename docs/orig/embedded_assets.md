# Embedded asset regions

Notes on `.data` regions of the retail `main.dol` that are **embedded binary assets**
rather than compiled C. dtk reconstructs their bytes byte-for-byte at build time from the
user-supplied disc under `orig/` (output lands in the gitignored `build/`); their bytes are
**never committed to this repo** (see *Why it isn't in source* below). They are catalogued
here so each reads as *identified and out-of-scope*, not as an unattributed gap in the
match. This mirrors `developer_artifacts.md`'s aim of turning anonymous blobs into named,
understood anchors.

## `0x802CC6A0`–`0x8030C6A0` — boot / loading-screen texture set (256 KB)

- **Identity (confirmed from code):** the game's **boot logo texture set** — three GameCube
  logo textures laid out back-to-back: the **Nintendo**, **Rareware**, and **Dolby Surround
  Pro Logic II** logos. Read at runtime by `initLoadingScreenTextures()` in
  `dll_0032_titlescreeninit.c`, which walks three `LoadingScreenTexture` records at
  `OSGetArenaHi() - 0x40000` (the arena copy of this region). Each record is a **0x60-byte
  header** (`width`@0x0A, `height`@0x0C `u16`; `format`@0x16 `u8`; pixels at `+0x60`) followed by
  the tile-swizzled image; `GXInitTexObj` is called with the header's own width/height/format, so
  the dimensions are **data-driven, not hardcoded** — which is how they were recovered exactly.
- **The three textures** are **offsets within** the single `gLoadingScreenTextures` blob, not
  separate symbols — the code reads them from the arena copy (below), never by per-logo symbol.
  Each record = 0x60 header + image:

  | # | logo | width×height | format | offset (addr) | record size |
  |---|---|---|---|---|---|
  | 0 | Nintendo | 376×104 | IA8 | `+0x0` (`0x802CC6A0`) | `0x131E0` |
  | 1 | Rareware | 128×160 | RGB5A3 | `+0x131E0` (`0x802DF880`) | `0xA060` |
  | 2 | Dolby Pro Logic II | 180×92 | CMPR (DXT1) | `+0x1D240` (`0x802E98E0`) | `0x22E0` |

  IA8/CMPR carry only intensity or a limited palette; the in-game yellow/gold is a render-time
  **TEV tint**, not stored in the texture. The three records fill `0x802CC6A0`–`0x802EBBC0`
  (`0x1F520`); everything after is `0xFF` padding.
- **Representation:** a single symbol `gLoadingScreenTextures` spanning the whole `0x40000` region.
  `src/main/boot_logo.c` `#include`s `gLoadingScreenTextures.inc`, which dtk auto-generates from the
  user's own disc via the `extract:` block in `config.yml` (see *Why it isn't in source*). The
  symbol carries `noreloc` in `symbols.txt` so dtk emits raw data and the object byte-matches — the
  region counts as 100% matched `game` data.
- **Naming:** `gLoadingScreenTextures` is the whole `0x40000` blob, **not** just the Nintendo
  record. `pi_dolphin.c:videoInit` memcpys all `0x40000` to arena-top and hands the same base to
  `GXInit` as the GPU FIFO, and indexes diagnostic format strings at `base + 0x4002c`. The three
  logos are offsets within it (table above), not separate symbols — no code references them
  individually.
- **Size:** `0x40000` (262,144 bytes) — three texture records (`0x1F520`) padded with `0xFF` to
  the `0x40000` GX-FIFO size.

### Runtime role
`videoInit()` copies the 256 KB to arena-top, then passes the same region to
`GXInit(base, 0x40000)` as the GPU command FIFO — the standard GameCube RAM-reuse pattern:
the initialized `.data` (the boot logos) is consumed at boot, and the RAM is then repurposed
as the write-only FIFO. `logGpuHang()` / `gpuErrorHandler()` also index diagnostic format
strings via `base + 0x4002c`.

### Note on the apparent "relocations"
dtk emits spurious `.4byte fn+offset` "relocations" inside the texture data — false positives, the
analyzer matching tiled/compressed image bytes against the `0x80xxxxxx` code-address range, not a
real pointer table. `gLoadingScreenTextures` is marked `noreloc` in `symbols.txt` so dtk emits the
whole `0x40000` region as raw data; the extracted C byte-array then matches the target.

### Why it isn't in source
It is an embedded copyrighted asset, so its 262,144 bytes are never *committed*. Instead, dtk's
`extract:` feature (config.yml) reconstructs it at build time: dtk pulls `gLoadingScreenTextures`
straight out of the user's own retail DOL under `orig/` (gitignored) and emits a gitignored
`build/GSAE01/include/gLoadingScreenTextures.inc` (a `unsigned char[]` byte-array); `boot_logo.c`
only `#include`s it. The repo holds one config entry and a one-line `.c` — never the content. This
is the standard dtk asset treatment (cf. marioparty4's `nintendoData` / `ank8x8_4b` extracts).

**Status: identified, reproduced via `config.yml` `extract:` + `boot_logo.c`, 100% matched — complete.**
Because it is reconstructed from the user's disc and byte-matches the target, it counts toward
`matched_data` like any other unit. Counting it — rather than leaving it an uncounted
`autogenerated` blob — is what lets the data metric reach 100%; excluding it caps the whole-DOL
figure below 100% forever, implying outstanding work that does not exist.

### Code-like edges (in scope — recovered as real C)
The compiler switch-jumptables and string literals bracketing the buffer are genuine source
and are attributed to their owning units, not part of the asset:
- `0x802CBE94`–`0x802CC6A0`: switch jumptables + asset-path/format strings
  (owners: `objprint_dolphin.c`, `pi_dolphin.c`).
- `0x8030C858`–`0x8030CF04`: inflate/deflate tables, a GX `IndTexCoordScale` table, and an
  OS debug format string (owner: `pi_dolphin.c`).


## Adjacent GPU-hang diagnostic strings

The `8030C6A0..8030C858` range immediately after the boot/FIFO allocation is a
440-byte diagnostic string block. The former autogenerated-data triage found no
direct references because `gpuErrorHandler` in `pi_dolphin.c` and `logGpuHang` in
`pi_videoinit.c` select its strings relative to `gLoadingScreenTextures`, starting
at offset `0x40000`.

Twelve native `char[]` definitions in `boot_logo.c` now reproduce the block.
The screen path uses the suspected-hang heading, the six-value GP status format,
and five status descriptions; the log path shares the format and uses five
newline-terminated descriptions. Their offsets are `+40000`, `+4002C`, `+40048`,
`+40068`, `+40090`, `+400B4`, `+400E4`, `+400FC`, `+4011C`, `+40144`, `+4016C`,
and `+4019C`. Each offset is relative to the boot/FIFO allocation, as in the
existing callers. The definitions use ordinary string initializers and natural
alignment, without explicit padding or another binary extraction.

The extended data-only split records the known contiguous representation; it
does not establish an original separate `boot_logo.c` translation unit. Consumer
pointer expressions are unchanged. The twelve symbols are retained through
`force_active`: the linker otherwise drops them because those consumer
relocations name the FIFO base, not the string symbols. Without retention, the
next string (`sThreadStateAttrSuspendFormat`) moves to `8030C6A0` and the strict
checksum fails. With retention, the complete retail DOL is byte-identical and
440 additional data bytes are matched and linked from C.
