...
void angleToVec2(int angle, float* sinOut, float* cosOut) {
    ...
    cosine = cosine * (cosine * (-2.575884e-31f * cosine + 1.3747608e-20f) + -2.8724248e-10f) + 1.0f;
...
void angleToVec2Precise(int angle, float* sinOut, float* cosOut) {
    ...
    float sine = fastCastS16ToFloat(&scaledAngleBits);
    float cosine = sine * sine;
    sine = sine * (cosine * (cosine * (-8.8444e-37f * cosine + 6.590636e-26f) + -2.2949214e-15f) + 0.000023968449f);
    cosine =
        cosine * (cosine * (cosine * (2.655e-42f * cosine + -2.632911e-31f) + 1.3751435e-20f) + -2.8724328e-10f) + 1.0f;
