 ...
 * The remainder of this translation unit is a v1.0 "drift" catalogue
 * shared with the sibling object DLLs: it defines the ObjectDescriptor
 * registration tables (and their config/colour/curve data) for a large
 * set of classes that physically live in this DLL image - kaldachompspit,
 ...
 *
 * The only real function bodies kept here are the siderepel callbacks,
 * the staticcamera callbacks (free/render/init) and the flamethrowerspe
 * render thunk fn_801719F8, plus the GX write-gather FIFO swipe helpers
 * at the tail.
 */
