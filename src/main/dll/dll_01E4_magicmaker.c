 ...
 * of the six creature type IDs in gMagicMakerCreatureTypeIds, and if fewer than 10
...
#define MAGICMAKER_CREATURE_TYPE_COUNT 6
#define MAGICMAKER_MAX_CREATURES       10    /* spawn only while fewer than this many exist */

u16 gMagicMakerCreatureTypeIds[6] = {0x2C4, 0x2CD, 0x2CE, 0x2CF, 0xB, 0xB};
    ...
    if (Obj_IsLoadingLocked() != 0)
    {
        if ((u32)mainGetBit(MAGICMAKER_SPAWN_GAMEBIT) != 0u)
        {
            ...
            for (i = 0; i < groupCount; i++)
            {
                ...
                {
                    if (*(s16*)(groupObj + 0x46) == gMagicMakerCreatureTypeIds[j])
                    {
                        matchCount++;
                    }
                }
            ...
            {
                objSetup = (char*)Obj_AllocObjectSetup(0x30, gMagicMakerCreatureTypeIds[randomGetRange(0, 5)]);
                if (objSetup != NULL)
                {
