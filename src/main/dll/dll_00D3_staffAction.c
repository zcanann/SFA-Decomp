...
void fn_80165B3C(int obj, int state)
{
    ...
    if (hitFound != 0)
    {
        ...
        fn_80166840((struct GameObject*)(obj), state, hitScratch.hit, end);
    }
    else
    {
...
void fn_80166840(struct GameObject *obj, int state, f32* hit, f32* end)
{
    ...
    scale = lbl_803E3020;
    stateX = scale * ((LandedArwingState*)state)->surfaceNormalX + (objX = ((GameObject*)obj)->anim.localPosX);
    stateY = scale * ((LandedArwingState*)state)->surfaceNormalY + (objY = ((GameObject*)obj)->anim.localPosY);
    stateZ = scale * ((LandedArwingState*)state)->surfaceNormalZ + (objZ = ((GameObject*)obj)->anim.localPosZ);
