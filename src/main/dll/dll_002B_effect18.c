                    ...
                    u8 modelId, void* extraArgs)
{
    ...
    if (gEffect18Progress0 > 1.0f) gEffect18Progress0 = 0.1f;
    gEffect18Progress1 += 0.0003f;
    if (gEffect18Progress1 > 1.0f) gEffect18Progress1 = 0.3f;
    ...
    case 0x70a:
        ...
        cfg.velocityY = 0.002f * (f32)(s32)
        randomGetRange(-0x14, 0x14);
        cfg.velocityZ = 0.002f * (f32)(s32)
        randomGetRange(-0x14, 0x14);
        cfg.scale = 0.0012f;
    ...
    case 0x714:
        ...
        if (extraArgs != 0)
        {
            ...
            cfg.initialAlpha *
                ((f32)(s32) * (int*)extraArgs / 255.0f);
        }
        cfg.velocityZ = 0.05f * (f32)(s32)
        randomGetRange(0x12, 0x14);
        cfg.scale = 9e-05f * (f32)(s32)
        randomGetRange(0x28, 0x3c);
    ...
    case 0x715:
        if (extraArgs != 0)
        {
            cfg.velocityX = 0.005f * (f32)(s32)
            randomGetRange(-0x19, 0x19);
            cfg.velocityY = 0.005f * (f32)(s32)
            randomGetRange(5, 0x32);
            cfg.velocityZ = 0.005f * (f32)(s32)
            randomGetRange(-0x19, 0x19);
            cfg.scale = 0.0015f;
    ...
    case 0x717:
        ...
        cfg.startPosX = thr * (0.1f * (f32)(s32)
        randomGetRange(-0x96, 0x96)
        )
        ;
        cfg.startPosY = thr * (0.1f * (f32)(s32)
        randomGetRange(0x64, 0x12c)
        )
        ;
        cfg.startPosZ = thr * (0.1f * (f32)(s32)
    ...
    case 0x723:
        {
            ...
            ;
            base = 0x41 - base;
            cfg.velocityX = 0.015f * (f32)(s32)
            randomGetRange(-base, base);
            cfg.velocityZ = 0.015f * (f32)(s32)
            randomGetRange(-base, base);
            cfg.scale = 0.002f * (f32)(s32)
            randomGetRange(6, 0xc);
...
void Effect18_func05(void)
{
    ...
    if (sum > 1.0f) gEffect18Progress2 = 0.1f;
    sum = gEffect18Progress3 + step;
    gEffect18Progress3 = sum;
    if (sum > 1.0f) gEffect18Progress3 = 0.3f;
