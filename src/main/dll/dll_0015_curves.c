...
void fn_800E5F1C(int obj, CurvesCollisionState* collision)
{
    ...
    for (i = 0; i < (int)hitCount; i++)
    {
        if ((s8)point->type != ROMCURVE_POINT_TYPE_WATER)
        {
            ...
            else if ((point->x >= (lbl_803E06AC + collision->points[0][1])) && (point->z < zero))
            {
...
void dll_15_func08(short* curveObj, CurvesCollisionState* state, uint updateValue, f32 step)
{
    ...
    collision = state;
    if ((state->flags & CURVES_COLLISION_STATE_ACTIVE) == 0)
    {
        return;
    }
    one = lbl_803E068C;
    ...
    if (collision->subtype == CURVES_COLLISION_SUBTYPE_OBJECT)
    {
        ...
        if (((state->flags & CURVES_COLLISION_STATE_LOCAL_POINTS) != 0) &&
        ...
        if (((state->flags & CURVES_COLLISION_STATE_HIT_SEGMENTS) != 0) &&
        ...
        if ((state->flags & 0x40000) == 0)
        {
            ...
            {
                ((GameObject*)curveObj)->anim.velocityY =
                    (f32)((f64)invStep * (f64)(((GameObject*)curveObj)->anim.worldPosY - ((GameObject*)linked)->anim.worldPosZ));
