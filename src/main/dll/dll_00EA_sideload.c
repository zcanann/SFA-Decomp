 ...
 * staff, fireball, flamethrowerspe, shield, mikabomb, staticCamera, curve,
 * etc.) that link from their own DLLs.
 */
...
void staticCamera_init(short* obj, int placementData, int addToGroup)
{
    u8* colorState;

    *obj = -*(short*)(placementData + 0x1c);
    obj[1] = -*(short*)(placementData + 0x1e);
    obj[2] = -*(short*)(placementData + 0x20);
    colorState = *(u8**)(obj + 0x5c);
    *colorState = *(u8*)(placementData + 0x19);
    *(float*)(colorState + 4) =
        (float)((double)CONCAT44(0x43300000, (uint) * (byte*)(placementData + 0x1a)) - DOUBLE_803e3e88);
    colorState[1] = 0;
    if (addToGroup == 0)
    {
