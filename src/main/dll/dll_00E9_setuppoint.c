 ...
 * load-time hook). The remainder of this translation unit is a v1.0
 * "drift" catalogue: it defines the ObjectDescriptor registration
 * tables (and their config/colour/curve data) for a large set of
 * object classes that physically live in this DLL image - kaldachompspit,
 * pinponspike, pollen / pollenfragment, mikabomb / mikabombshadow,
 * staticcamera, gcbaddieshield, baddieinterestp, animatedobj, dim2roofrub,
 * depthoffieldpoint, staff, fireball, flamethrowerspe, shield, curve,
 * restartmarker, dll_F7 and checkpoint4. Each descriptor wires that
 * class's initialise/release/init/update/hitDetect/render/free/typeId/
