...
int wclevelcont_func10(int obj, s16 a, s16 b, f32* outX, f32* outZ, int dx, int dy)
{
    ...
    if (dx != 0)
    {
        ...
        if (dx == -1)
        {
            ...
            fr = lbl_803E6DD0 + px;
            *outX = lbl_803E6DB4 + (fr + lbl_803E6DBC);
            fr = lbl_803E6DD4 + pz;
            *outZ = lbl_803E6DB4 + (fr + (f32)(bi * 48));
            a += 1;
            limit = 8;
        }
        else
        {
            ...
            mapGetBlockOriginForPos(((GameObject*)obj)->anim.localPosX, ((GameObject*)obj)->anim.localPosY,
                                    ((GameObject*)obj)->anim.localPosZ, &px, &pz);
            fr = lbl_803E6DD0 + px;
            *outX = lbl_803E6DB4 + (fr + lbl_803E6DA8);
            fr = lbl_803E6DD4 + pz;
            *outZ = lbl_803E6DB4 + (fr + (f32)(bi * 48));
            a -= 1;
            limit = -1;
        ...
        for (i = a; i != limit; i -= dx)
        {
            if (lbl_803AD2D8[i][b] != 0)
            {
                if (lbl_803AD2D8[i][b] <= 4)
                {
                    f32 pz, px, fr;
                    i += dx;
                    mapGetBlockOriginForPos(((GameObject*)obj)->anim.localPosX,
                                            ((GameObject*)obj)->anim.localPosY, ((GameObject*)obj)->anim.localPosZ, &px, &pz);
                    fr = lbl_803E6DD0 + px;
                    *outX = lbl_803E6DB4 + (fr + (f32)((s16)i * 48));
                    return 1;
                }
                {
                    f32 pz, px, fr;
    ...
    else
    {
        ...
        if (dy == -1)
        {
            ...
            mapGetBlockOriginForPos(((GameObject*)obj)->anim.localPosX, ((GameObject*)obj)->anim.localPosY,
                                    ((GameObject*)obj)->anim.localPosZ, &px, &pz);
            fr = lbl_803E6DD0 + px;
            *outX = lbl_803E6DB4 + (fr + (f32)(ai * 48));
            fr = lbl_803E6DD4 + pz;
            *outZ = lbl_803E6DB4 + (fr + lbl_803E6DBC);
            b += 1;
            limit = 8;
        }
        else
        {
            ...
            *outX = lbl_803E6DB4 + (fr + (f32)(ai * 48));
            fr = lbl_803E6DD4 + pz;
            *outZ = lbl_803E6DB4 + (fr + lbl_803E6DA8);
            b -= 1;
            limit = -1;
        }
            ...
            if (lbl_803AD2D8[a][i] != 0)
            {
                if (lbl_803AD2D8[a][i] <= 4)
                {
                    ...
                    mapGetBlockOriginForPos(((GameObject*)obj)->anim.localPosX,
                                            ((GameObject*)obj)->anim.localPosY, ((GameObject*)obj)->anim.localPosZ, &px, &pz);
                    fr = lbl_803E6DD4 + pz;
                    *outZ = lbl_803E6DB4 + (fr + (f32)((s16)i * 48));
                    return 1;
                }
                {
                    f32 pz, px, fr;
