...
int drshackle_updateSwingBlend(int obj, int state)
{
    ...
    {
        s32 val = yawDelta / DRSHACKLE_ANGLE_STEP;
        if (val < -DRSHACKLE_SWING_BLEND_LIMIT)
        {
            val = -DRSHACKLE_SWING_BLEND_LIMIT;
        }
        else if (val > DRSHACKLE_SWING_BLEND_LIMIT)
        {
            val = DRSHACKLE_SWING_BLEND_LIMIT;
        }
        *(f32*)(state + DRSHACKLE_SWING_BLEND_OFFSET) = (f32)(-val);
    }
    ...
    *(f32*)(state + DRSHACKLE_SWING_BLEND_OFFSET) =
    ...
    {
        f32 fa = *(f32*)(state + DRSHACKLE_SWING_BLEND_OFFSET);
        *(f32*)(state + DRSHACKLE_SWING_BLEND_OFFSET) =
            (fa < lbl_803E5B70)
                ? lbl_803E5B70
                : ((fa > lbl_803E5AEC) ? lbl_803E5AEC : fa);
    }
...
int drshackle_updateAttachedPosition(int obj, int state)
{
    ...
    val2 = objPosToMapBlockIdx(((GameObject*)obj)->anim.localPosX, ((GameObject*)obj)->anim.localPosY,
    ...
    if (val2 > -1)
    {
        if (flags->positionAnchored == 0)
        {
            ...
            *(f32*)(state + DRSHACKLE_LAST_PITCH_OFFSET) = -fn_801EA678(obj, state);
            val = (*(int (**)(int, int, f32, u8, int, int))(*gCheckpointInterface + 0x18))(
            ...
            *(f32*)(*(int*)(obj + DRSHACKLE_MODEL_OFFSET) + 0x24) = ((GameObject*)obj)->anim.worldPosZ;
            ...
            if (*(u8*)(state + DRSHACKLE_FLOOR_ADJUST_FLAG_OFFSET) == 0)
            {
                hitDetectFn_800658a4(obj, ((GameObject*)obj)->anim.localPosX,
                                     ...
                                     &tmp, 0);
                ((GameObject*)obj)->anim.localPosY = ((GameObject*)obj)->anim.localPosY - tmp;
