...
void fn_801EC1AC(int obj, int state)
{
    ...
    clampedRate = (rate < lbl_803E5C2C) ? lbl_803E5C2C
    ...
    *(f32 *)(state + 0x430) = clampedRate * timeDelta + *(f32 *)((int)state + 0x430);
    ...
    if (flags->b4 != 0) {
        ...
        if (v49c >= target) {
            f32 nv = -vy53c;
            target = (nv < -v49c * oneOverTimeDelta) ? -v49c * oneOverTimeDelta
                                                    : ((nv > target) ? target : nv);
        } else {
            target = (vy53c < target)
                        ? target
    ...
    if (flags->b7 != 0) {
        *(f32 *)(state + 0x584) =
            (-*(f32 *)(state + 0x570)) * timeDelta + *(f32 *)(state + 0x584);
        {
            ...
            *(f32 *)(state + 0x584) = (v < lbl_803E5C30) ? lbl_803E5C30
