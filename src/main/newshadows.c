...
static inline void newshadows_initEntries(NewShadowEntry* entries) {
    int i;
    for (i = 0; i < NEW_SHADOW_ENTRY_CAPACITY; i++) {
        entries[i].isActive = 0;
        entries[i].state = 1;
    }
}
...
void allocLotsOfTextures(void) {
    ...
    newshadows_initEntries(gNewShadowEntries);
...
void newshadows_releaseTextureEntry(void* textureEntry) {
    int i;
    /* Retail's byte scan runs four records past the 33-entry array. The last
       four flag accesses touch the following texture-pointer table. */
    for (i = 0; i < 0x25; ++i) {
