...
void initSkyStars(void) {
    ...
    for (constellationPointIndex = 0; constellationPointIndex < SKY_STAR_CONSTELLATION_POINT_COUNT;
         constellationPointIndex++) {
        do {
            starPosition.x = (int)randomGetRange(-5000, 5000);
            starPosition.y = (int)randomGetRange(-5000, 5000);
            starPosition.z = (int)randomGetRange(-5000, 5000);
        ...
        constellation[constellationPointIndex].x = starPosition.x;
        constellation[constellationPointIndex].y = starPosition.y;
        constellation[constellationPointIndex].z = starPosition.z;
    }
    ...
    for (displayListIndex = 0; displayListIndex < SKY_STAR_DISPLAY_LIST_COUNT; displayListIndex++) {
        gNewCloudStarDisplayLists[displayListIndex] = mmAlloc(SKY_STAR_DISPLAY_LIST_BUFFER_SIZE, 0x7f7f7fff, 0);
            ...
            if (randomGetRange(0, 9) < 5) {
                ...
                if (ax > 3750.0f) {
                    PSMTXRotRad(rotationA, 0x79,
                                (0.015f * (2.0f * (3.142f * randomGetRange(-0x8000, 0x8000)))) / 32768.0f);
