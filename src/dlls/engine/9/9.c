...
void renderClouds(int a, int b, int c, int d) {
    ...
    if (cloudT > 0.0f) {
        ...
        GXTex2f32(c0, c0);
        v = -gCloudActionGlareQuadSize[0];
        GXPos3f32(((const f32*)gCloudActionGlareQuadSize)[0], v, c0);
        GXTex2f32(c1, c0);
        v = gCloudActionGlareQuadSize[0];
        GXPos3f32(((const f32*)gCloudActionGlareQuadSize)[0], v, c0);
        GXTex2f32(c1, c1);
...
ResourceDescriptorCallbacks14 cloudaction_funcs = {
    ...
    {(ResourceDescriptorCallback)cloudaction_initialise, (ResourceDescriptorCallback)cloudaction_release, 0x00000000,
