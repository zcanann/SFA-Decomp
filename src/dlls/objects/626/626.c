...
int hightop_stateHandler10(GameObject* obj, HighTopRuntime* stateArg) {
    ...
    if (Vec_distance(&Obj_GetPlayerObject()->anim.worldPosX, &obj->anim.worldPosX) > 700.0f) {
        if (randomChanceOneIn(500) != 0) {
            ...
            i = 0;
            while (gHighTopIdleSequenceWeights[i] < roll) {
...
int hightop_stateHandler04(GameObject* obj, HighTopRuntime* stateArg) {
    ...
    if (mainGetBit(0x62b) != 0) {
        ...
        *(s8*)&state->substate = -1;
        ...
        state->flagsC49.b1 = 0;
        curve->initFromCurveId(&state->curveWalker, obj, 0x3463a, (curve = *gRomCurveInterface));
        state2 = obj->extra;
    ...
    if (player != 0 && (((dy = player->anim.localPosY - obj->anim.localPosY) >= 0.0f ? dy : -dy) < 30.0f ||
                        ((dy = player->anim.localPosY - obj->anim.localPosY) >= 0.0f ? dy : -dy) > 300.0f)) {
        state->lookController.modeBits |= 1;
...
u8 gHighTopConfigTable[0x80] = {
    ,
};
...
int hightop_stateHandler01(GameObject* obj, HighTopRuntime* stateArg) {
    ...
    if (*(f32*)&stateArg->baddie.trackedObj > 0.0f && stateArg->baddie.inputMagnitude > 0.0f) {
        return 3;
    }
    return 0;
}
...
void HighTop_render(void* obj, int p2, int p3, int p4, int p5, char visible) {
    ...
    if (visible != 0) {
        ...
        ObjPath_GetPointWorldPosition((GameObject*)obj, 2, &runtime->pathPoint2X, &runtime->pathPoint2Y,
                                      &runtime->pathPoint2Z, 0);
        ObjPath_GetPointWorldPositionArray((GameObject*)obj, 3, 4, runtime->pathPointWorldPositions);
        ObjPath_GetPointWorldPosition((GameObject*)obj, 0, &runtime->pathPoint0X, &runtime->pathPoint0Y,
                                      &runtime->pathPoint0Z, 0);
        runtime->flagsC49.b5 = 1;
        dll_2E_setTargetFromPathPoint((GameObject*)obj, &runtime->lookController, 0);
        if (runtime->flagsC49.b1 != 0) {
            GameObject** shackles = objGetAllOfType(DRSHACKLE_OBJGROUP, &count);
            for (i = 0; i < count; i++) {
                int idx = DRSHACKLE_INTERFACE(shackles[i])->getAttachSlot(shackles[i]);
                void (*dispatch)(GameObject*, void*, int, int, int, int, int) =
                    DRSHACKLE_INTERFACE(shackles[i])->renderAtPathPoint;
                dispatch(shackles[i], obj, gHighTopTuning.shacklePathPoints[idx], p2, p3, p4, p5);
            }
        }
    } else {
