...
void dfptargetblock_resolveCollisionPoints(GameObject* obj, DfpTargetBlockState* state) {
    ...
    for (i = 0; i < state->floorPointCount; i++) {
        probe[0] = state->floorPoints[i].x + obj->anim.localPosX;
        originalX = probe[0];
        probe[1] = state->floorPoints[i].y + obj->anim.localPosY;
        probe[2] = state->floorPoints[i].z + obj->anim.localPosZ;
        originalZ = probe[2];
...
static inline void dfptargetblock_resetToHome(GameObject* obj, ObjPlacement* home, DfpTargetBlockState* state) {
    f32 zero;
...
static inline void dfptargetblock_checkSettled(GameObject* obj, DfpTargetBlockState* state, f32 threshold) {
    f32 dx;
    f32 dz;
