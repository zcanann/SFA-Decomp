...
static inline int bossdrakor_clampStep(int frames, int delta) {
    int r;
    if (delta < -(frames << 8)) {
        r = -(frames << 8);
    } else {
        int u = (delta > (frames << 8)) ? (frames << 8) : delta;
        r = u;
    }
    return r;
}
void bossdrakor_updateHeadTracking(GameObject* obj, BossDrakorState* drakorState) {
    ...
    if (neck != NULL) {
        neck[0] += (s16)bossdrakor_clampStep(framesThisStep, (s16)-neck[0]);
                ...
                if ((u8)Obj_CanSetupObject() != 0) {
                    ...
                    if (player != NULL) {
                        ...
                        if (missile != NULL) {
                            ...
                            PSVECSubtract(&player->anim.localPos, &s->homePos, &vecA);
                            PSVECSubtract(&target, &s->homePos, &vecB);
                            PSVECNormalize(&vecA, &vecA);
                            spd = s->missileLeadFactor * PSVECDotProduct(&player->anim.velocity, &vecA) +
                                  s->missileBaseSpeed;
...
void bossdrakor_render(GameObject* obj, int p2, int p3, int p4, int p5, s8 vis) {
    ...
    ObjPath_GetPointWorldPosition(obj, 0, &s->homePosX, &s->homePosY, &s->homePosZ, 0);
    if (s->lightObj != NULL) {
...
static inline void bossdrakor_updateEffects(GameObject* obj, BossDrakorState* drakorState,
    ...
    for (i = 0, eventCursor = (s8*)animEvents; i < animEvents->triggerCount; i++) {
