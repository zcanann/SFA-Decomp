...
#define TRICKY_COMMAND_FROM_STATE_BASE(base) \
    ((TrickyCommand*)((base) + offsetof(TrickyState, commands)))
#define TRICKY_COMMAND_TTL_FROM_STATE_BASE(base) \
...
int moveTricky(GameObject* obj, f32* targetPos) {
    ...
    if (moveSpeed >= 0.05f) {
        ...
        if (skeetla_isInWater(state) != 0) {
            trickyRequestMove(obj, 7, TRICKY_TINY_MOVE_BLEND_SPEED, 0x2000000);
            ...
            if (moveSpeed > 2.5f) {
                ...
                trickyRequestMove(obj, 0x30, TRICKY_TINY_MOVE_BLEND_SPEED, 0x3000000);
            } else if (moveSpeed > 1.0f) {
                trickyRequestMove(obj, 5, TRICKY_TINY_MOVE_BLEND_SPEED, 0x3000000);
            } else if (moveSpeed > TRICKY_FAST_WALK_MOVE_THRESHOLD) {
                trickyRequestMove(obj, 4, TRICKY_TINY_MOVE_BLEND_SPEED, 0x3000000);
            } else if (moveSpeed > TRICKY_SLOW_WALK_MOVE_THRESHOLD) {
                trickyRequestMove(obj, 2, TRICKY_TINY_MOVE_BLEND_SPEED, 0x3000000);
            } else {
                trickyRequestMove(obj, 1, TRICKY_TINY_MOVE_BLEND_SPEED, 0x3000000);
            }
        ...
        if ((state->stateFlags & 0x100000) != 0) {
            if (skeetla_isInWater(state) != 0) {
                ...
                trickyRequestMove(obj, animId, TRICKY_TURN_MOVE_BLEND_SPEED, 0x1000100);
            }
        }
                        ...
                        if (objectWalkGroup != 0) {
                            ...
                            if (objectWalkGroup == 0 &&
                                ...
                                i = isPointWithinPatchGroup(&obj->anim.worldPosX, state->activeWalkGroup,
                                                            patchGroupForCheck);
                                trickyReportError(debugStrings + TRICKY_DBG_PATCH_ROUTE_ERROR, patchGroupForCheck, targetWalkGroup,
...
void trickyGrowl(GameObject* obj, TrickyState* trickyState) {
    ...
    case TRICKYGROWL_WINDUP:
        ...
        if (trickyUpdateMovementState(obj, 30.0f, trickyState) == 0) {
            ...
            trickyRequestMove(obj, 0x33, TRICKY_LAND_MOVE_BLEND_SPEED, 0x4000000);
    ...
    case TRICKYGROWL_DIG_START:
        ...
        if (trickyUpdateMovementState(obj, 25.0f, trickyState) == 0) {
            ...
            trickyRequestMove(obj, 0x34, TRICKY_LAND_MOVE_BLEND_SPEED, 0x4000000);
...
void tricky_fetchBall(GameObject* obj, TrickyState* state) {
        ...
        if (sidekickBall_isHeldOrMoving(state->scratch700.obj) != 0) {
            ...
            if (status == 0) {
                if (state->scratch704.f > 0.0f) {
                    ...
                    trickyRequestMove(obj, 16, TRICKY_FAST_MOVE_BLEND_SPEED, 0x4000000);
    ...
    case 6:
        if ((obj)->anim.currentMoveProgress >= 0.65f) {
            ...
            sidekickBall_launch(state->scratch700.obj, obj, -mathSinf(TRICKY_PI * (f32)(s32) * (short*)obj / TRICKY_ANGLE_HALF_TURN_UNITS),
                                1.0f, bob);
            state->substate = 2;
        }
        break;
...
void trickyDigTunnel(GameObject* obj, TrickyState* state) {
    ...
    case 4:
        ...
        spd = GROUND_ANIMATOR_INTERFACE(state->followObj)
                  ->applyPress(state->followObj, obj);
        obj->anim.localPosX =
    ...
    case 5:
        trickyDebugPrint((char*)(base + TRICKY_DBG_DIGTUNNEL_TOEND1),
                         Vec_xzDistance(&obj->anim.worldPosX,
...
void Tricky_update(GameObject* obj) {
    ...
    }
    {
        ...
        trickyDebugPrint(base + TRICKY_DBG_SIDECOMMAND_HITS, flagsByte & 1, flagsByte & 2, flagsByte & 4,
                         flagsByte & 8, flagsByte & 0x10,
