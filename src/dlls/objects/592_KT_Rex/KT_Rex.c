...
int ktrex_shouldAdvanceArenaPhase(void) {
    ...
    if ((currentMask == 1 && (gKTRexState->activeLaneMask & 8)) || (currentMask == 4 && (activeMask & 1)) ||
        (currentMask == 2 && (activeMask & 4)) || (currentMask == 8 && (activeMask & 2))) {
        return 1;
    }
    return 0;
}
...
int ktrex_stateHandlerA11(GameObject* obj, GroundBaddieState* runtime) {
    ...
    f4 = gKTRexState->rowBZ[phase] - gKTRexState->rowAZ[phase];
...
int ktrex_stateHandlerA10(GameObject* obj, GroundBaddieState* runtime) {
    ...
    if ((s8)runtime->baddie.moveJustStartedB != 0) {
        ...
        {
            u8* row = (u8*)p + 0x38;
            runtime->baddie.animSpeedC = *(f32*)(row + gKTRexState->laneIndex * 4) / 1000.0f;
        }
    }
    ...
    if (gKTRexState->stateTimer <= 0.0f && gKTRexState->lastPhase == phase &&
        ((laneBit == 0 && gKTRexState->laneLerpT >= 0.75f) || (laneBit != 0 && gKTRexState->laneLerpT <= 0.25f))) {
...
int ktrex_stateHandlerA02(GameObject* obj, GroundBaddieState* runtime) {
    ...
    flags = gKTRexState->timerFA;
    flag1 = flags & 1;
    if (gKTRexState->laneIndex == 0 && (phase = gKTRexState->phaseCounter) >= 2 && (flags & 0x20) == 0 &&
        ((flag1 == 0 && gKTRexState->laneLerpT >= 0.7f) || (flag1 != 0 && gKTRexState->laneLerpT <= 0.3f))) {
        idx = phase >> 1;
        pb = (u8*)p;
...
int ktrex_stateHandlerB08(GameObject* obj, GroundBaddieState* runtime) {
    if (runtime->baddie.moveJustStartedA != 0) {
        ...
        runtime->baddie.moveSpeed = 0.0017f + 0.0012f * (f32)(int)(gKTRexState->phaseCounter >> 1);
...
int ktrex_stateHandlerB03(GameObject* obj, GroundBaddieState* runtime) {
    ...
    if (dir != 0) {
        (obj)->anim.rotX = 32768.0f * (obj)->anim.currentMoveProgress + (f32)(int)gKTRexState->homeYaw;
    } else {
        (obj)->anim.rotX = (f32)(int)gKTRexState->homeYaw - 32768.0f * (obj)->anim.currentMoveProgress;
    }
    return 0;
}
...
int ktrex_stateHandlerB02(GameObject* obj, GroundBaddieState* runtime) {
    ...
    Matrix_TransformPoint(mtx, runtime->baddie.animSpeedB, 0.0f, -runtime->baddie.animSpeedA, &(obj)->anim.velocityX,
                          &tmpY, &(obj)->anim.velocityZ);
    if (dir != 0) {
        (obj)->anim.rotX = 16384.0f * (obj)->anim.currentMoveProgress + (f32)(int)gKTRexState->homeYaw;
    } else {
        (obj)->anim.rotX = (f32)(int)gKTRexState->homeYaw - 16384.0f * (obj)->anim.currentMoveProgress;
    }
    return 0;
}
...
void ktrex_updateContactEffects(GameObject* obj, GroundBaddieState* runtime) {
    ...
    if (gKTRexRuntime->glowAlpha > 0.0f) {
        gKTRexRuntime->glowAlpha = timeDelta * gKTRexRuntime->glowRate + gKTRexRuntime->glowAlpha;
        if (gKTRexRuntime->glowAlpha < 0.0f) {
            gKTRexRuntime->glowAlpha = 0.0f;
        } else if (gKTRexRuntime->glowAlpha > 120.0f) {
            gKTRexRuntime->glowAlpha = 120.0f - (gKTRexRuntime->glowAlpha - 120.0f);
    ...
    if (runtime->baddie.hitPoints != 0 && (hitType == 3 || hitType == 2) && (gKTRexState->timerFA & 0x10) != 0 &&
        hit == 5) {
        ...
        (*gKTRexResource)
            ->spawn(obj, 0, (PartFxSpawnParams*)&gKTRexEffectSpawnWork, 1, -1, (StaffCollisionColorArgs*)msg.w);
...
int ktrex_updateArenaPathProgress(GroundBaddieState* runtime) {
    ...
    if ((gKTRexState->laneLerpT > gKTRexLaneTuning.speedMax[gKTRexState->laneIndex] && speed > 0.0f) ||
        (gKTRexState->laneLerpT < gKTRexLaneSpeedMin[gKTRexState->laneIndex] && speed < 0.0f)) {
        if (dir != 0) {
            phase--;
            if (phase < 0) {
                phase = 3;
    ...
    gKTRexState->posX =
        gKTRexState->laneLerpT * (gKTRexState->rowBX[phase] - gKTRexState->rowAX[phase]) + gKTRexState->rowAX[phase];
    gKTRexState->posY =
        gKTRexState->laneLerpT * (gKTRexState->rowBY[phase] - gKTRexState->rowAY[phase]) + gKTRexState->rowAY[phase];
    gKTRexState->posZ =
        gKTRexState->laneLerpT * (gKTRexState->rowBZ[phase] - gKTRexState->rowAZ[phase]) + gKTRexState->rowAZ[phase];
    return changed;
}
...
void ktrex_update(GameObject* obj) {
    ...
    if (runtime->baddie.targetObj != NULL) {
        ...
        for (index[0] = 0; index[0] < 3; index[0]++) {
            dp[index[0]] = (&player->anim.worldPosX)[index[0]] - (&obj->anim.worldPosX)[index[0]];
        }
    ...
    laneMask[0] = 0;
    index[0] = laneMask[0];
    enabledBit = gKTRexLaneEnabledGameBits;
    for (; index[0] < 4; index[0]++) {
        if (mainGetBit(*enabledBit) != 0) {
            laneMask[0] |= 1 << index[0];
        }
        enabledBit++;
    }
    gKTRexState->activeLaneMask = laneMask[0];
    player = runtime->baddie.targetObj;
    {
    ...
    laneMask[0] = 0;
    index[0] = laneMask[0];
    modeBit = gKTRexLaneModeGameBits;
    flags = gKTRexState->currentLaneMask;
    for (; index[0] < 4; index[0]++) {
        mm = 1 << index[0];
        if ((flags & mm) != 0 && mainGetBit(*modeBit) != 0) {
            laneMask[0] |= mm;
        }
        modeBit++;
    }
    gKTRexState->laneMode = laneMask[0];
    (*gBaddieControlInterface)
        ->processMessages(obj, runtime, &gKTRexRuntime->routeNav, gKTRexRuntime->gameBitB, &gKTRexRuntime->subMode, 2,
                          2, 0);
    ...
    (*gPlayerInterface)->update((void*)obj, runtime, timeDelta, timeDelta, gKTRexStateHandlersB, gKTRexStateHandlersA);
