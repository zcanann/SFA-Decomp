...
void SnowBike_DrawTrails(GameObject* obj, SnowBikeState* state) {
    ...
    for (i = 0, p = (u8*)state; i < SNOWBIKE_TRAIL_COUNT; p += sizeof(SnowBikeTrail), i++) {
        trail = (SnowBikeTrail*)(p + offsetof(SnowBikeState, trails));
...
void SnowBike_UpdateTrails(GameObject* obj, SnowBikeState* state) {
    ...
    for (i = 0, k = 0, k2 = 3; i < 3; k += 6, k2 += 6, i++) {
        ...
        Matrix_TransformPoint(matrix, localPoints.points[end], localPoints.points[end + 1], localPoints.points[end + 2],
                              &pos[1].x, &pos[1].y, &pos[1].z);

        hitDetected = 0;
        for (endpointIndex = 0; endpointIndex < 2; endpointIndex++) {
            hitCount =
                trackGetHeight(obj, pos[endpointIndex].x, pos[endpointIndex].y, pos[endpointIndex].z, &hits, 0, 0x20);
...
static void SnowBike_ClampWindVolume(volume, limit) f32 volume;
f32 limit;
{
    if (volume < 0.0f) {
...
f32 SnowBike_GetRouteIntensity(GameObject* obj, SnowBikeState* state) {
    ...
    if ((gSnowBikeLeaderRouteRank == -1) ||
        ...
        if (gSnowBikeLeaderRouteRank == -1) {
            ...
            leaderMetric =
                100.0f * gSnowBikeLeaderRankItem.item.linkDepth + 100.0f * gSnowBikeLeaderRankItem.item.routeProgress;
...
int SnowBike_UpdateSwingBlend(GameObject* obj, SnowBikeState* state) {
    ...
    routeDone =
        (*gCheckpointInterface)->advanceRoute(&state->routeCursor, &state->routeState, speed, state->routeMode, 1, 0);
    ...
    yawDelta =
        getAngle(obj->anim.localPosX - state->routeCursor.x, obj->anim.localPosZ - state->routeCursor.z) & 0xffff;
...

    {
        ...
        if (state->localVel.z < maxSpeed || yawDelta > SNOWBIKE_SWING_ANGLE_RETURN_LIMIT ||
            ...
            state->buttonsHeld = PAD_BUTTON_A;
        }
    }
    return 1;
}
...
void SnowBike_UpdateLiftSway(GameObject* obj, SnowBikeState* state) {
    ...
    if ((state->buttonsHeld & PAD_BUTTON_A) != 0) {
        flags->accelerating = 1;
    } else {
        flags->accelerating = 0;
    }

    if ((state->buttonsHeld & PAD_BUTTON_B) != 0) {
        flags->braking = 1;
    } else {
        flags->braking = 0;
    }
    ...
    if (flags->airborne != 0) {
        ...
        obj->anim.rotY += state->pitchVel * timeDelta;
    }

    if (flags->cpuDriven == 0) {
        cameraArgs[0] = state->yawVel;
        cameraArgs[1] = state->localVel.z;
        cameraArgs[2] = obj->anim.rotZ;
        cameraArgs[3] = obj->anim.rotY;
...
s32 SnowBike_isAtRankGate(GameObject* obj) {
    ...
    return gSnowBikeLeaderRouteRank - 1 - rank == 0;
}
...
void SnowBike_hitDetect(GameObject* obj) {
    ...
    if (state->mountState == SNOWBIKE_MOUNT_STATE_RIDING) {
        ...
        obj->anim.rotY += state->wobbleY;
        obj->anim.rotZ += state->rollOffset + state->wobbleZ;
    }
    ...
    case SNOWBIKE_MOUNT_STATE_RIDING:
        ...
        if (state->flags.cpuDriven) {
            if (SnowBike_UpdateAttachedPosition(obj, state) != 0) {
                ...
                SnowBike_buildOrientationMatrices(obj, state);
                if (state->impactTimer) {
...
void SnowBike_init(GameObject* obj, SnowBikePlacement* params, int flag) {
    ...
    state->raceGameBits = (s16*)((u32)pathSetup[0] + SNOWBIKE_SETUP_GAMEBITS_OFFSET + state->bikeType * 6);
    if (state->bikeType == 0) {
        if (!state->flags.cpuDriven) {
