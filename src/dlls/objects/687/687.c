...
void tree_spawnApple(GameObject* obj, TreeState* state, s8 index) {
    ...
    if ((u8)Obj_CanSetupObject()) {
        ...
        ts->apples[idx] = objSetupObject(&applePlacement->base, 5, obj->anim.mapEventSlot, -1, obj->anim.parent);
    }
}
    ...
    if (obj->userData2 != 0) {
        ...
        for (i = 0; i < TREE_APPLE_COUNT; i++) {
            if (state->apples[i] == NULL) {
                ...
                if (APPLE_ON_TREE_INTERFACE(state->apples[i])->getAnimState(state->apples[i]) > 3) {
                    state->apples[i] = 0;
                } else {
                    APPLE_ON_TREE_INTERFACE(state->apples[i])->setPosition(state->apples[i], &ts->applePositions[i][0]);
                }
            }
        }
    }
...
void tree_render(GameObject* obj, int p2, int p3, int p4, int p5, s8 visible) {
    ...
    if (visible != 0) {
        ...
        if (state->flags & TREE_FLAG_APPLES) {
            for (i = 0; i < TREE_APPLE_COUNT; i++) {
                ObjPath_GetPointWorldPosition(obj, i, &state->applePositions[i][0], &state->applePositions[i][1],
        ...
        if (state->flags & TREE_FLAG_HIT_ENABLED) {
            ...
            if (hit != 0 && hit != OBJHITREACT_COLLISION_SKIP_REACTION && state->hitCooldownTimer <= 0.0f) {
                ...
                if (state->flags & TREE_FLAG_BURST_MODE_MASK) {
                    ...
                    *(offsetPtr = &burstParams.posX) =
                        effectScale * gTreeEffectBursts[state->burstProfileIndex].offset.x;
                ...
                if (state->flags & TREE_FLAG_APPLES) {
                    if (hit != 0) {
                        for (i = 0; i < TREE_APPLE_COUNT; i++) {
                            if (state->apples[i] != NULL) {
                                if (APPLE_ON_TREE_INTERFACE(state->apples[i])->getAnimState(state->apples[i]) > 1) {
...
void tree_init(GameObject* obj, TreePlacementPrefix* setup) {
    ...
    case TREE_OBJECT_SNOW_TREE_3:
        ...
        ObjHitbox_SetCapsuleBounds(&obj->anim, (int)(6.0f * obj->anim.rootMotionScale), -0x5, 0x64);
        break;
