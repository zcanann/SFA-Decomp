...
float cosf(float angle) {
    ...
    quadrant <<= 1;
    result =
        (((__sincos_poly[0] * reducedSquared + __sincos_poly[2]) * reducedSquared + __sincos_poly[4]) * reducedSquared +
         __sincos_poly[6]) *
            reducedSquared +
        __sincos_poly[8];
...
float sinf(float angle) {
    ...
    if (fabsf(reducedAngle) < __epsilon) {
        quadrant <<= 1;
        return __sincos_on_quadrant[quadrant] + (__sincos_on_quadrant[quadrant + 1] * reducedAngle * __sincos_poly[9]);
    }
