...
- Local **declaration order** sets saved-register homes (first-declared top-loaded value → highest reg). Reorder decls to swap registers. Only **genuine locals with disjoint live ranges** participate — if a competing value is a coalesced parameter copy or a call result, every permutation is bit-identical, which is why this reads as inert when tested on the wrong operands. Two further dead classes: a value with **no named local behind it** (a register copy, a spill reload, an array base address) is unreachable by any declaration; and locals whose live ranges **overlap** across the whole body are ranked by use, not by declaration. Sweep with `tools/permsweep.py` (gates on bytes via `tools/fnbytes.py` — never on tool silence).
