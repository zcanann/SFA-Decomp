...
- **★ OWNER PRINCIPLE — DISTRUST RAW DEREFS/CASTS; the real 2002 code used STRUCTS/UNIONS EVERYWHERE.** Raw
  ...
  higher-yield than the coloring grind where it changes codegen. ⚠️ SCOPE PINNED (HunterB + Validator, in-tree
  ...
  • SINGLE-INDEX MULTI-FIELD WRITE (`*(T*)(base+idx*stride+fieldoff)`, one index, fields written once) → the RAW
    ...
    the offset-0 field). So DON'T struct-convert these — raw is the byte-optimal AND faithful-enough form.
  • STATE/PARAM-RELATIVE field access → BYTE-NEUTRAL (base already a held reg; caching `T* arr=(T*)(state+off)`
    REGRESSES, adds a saved-reg competitor). • FIXED-ADDRESS VOLATILE (GX FIFO) → NEUTRAL.
  • LOCAL-VAR coloring nuts (value-reuse, index coloring) → NOT a struct issue at all; it's a real allocator
    rule (name it).
  RULE: A/B EACH candidate; the % win is the global-base WALKED-across-calls pattern specifically. Faithful-but-
  neutral/worse conversions are a source-quality policy call, not a match win (many raw casts are byte-optimal).
...
136. **Strength-reduced loop COUNTER/WALKER coloring is a SOURCE lever — two clean forms, pick by
    ...
    keeping the reg live longer) → retail `mr`-reuse. ⚠️ SHARPENED (Validator add-live-range experiment): that
    use-count is RETAIL's CORRELATION across two DIFFERENT fns — it is NOT a generic register-count/pressure
    threshold for OURS. Adding 1-8 generic live-across-call ranges to /tmp/vCP does NOT flip count=1 `li`→`mr`;
    AND the FULL faithful curves_getPos reconstruction is opcode-IDENTICAL to target except that ONE `li`-vs-`mr`
    (same GC/2.0 -O4,p). So the exact trigger is a SUBTLE SOURCE-OR-OPT difference affecting ONLY that one
    materialization — definitively NOT generic pressure/count. THE OPEN WORK: BISECT the full reconstruction vs a
    variant until the 1 instr flips → name the SPECIFIC construct (a coalesce-eligibility fact), not a count.
    Lower-priority (0.4%/fn). UNIFIED with the #147 copy-PULL bed (/tmp/v147/faithful.c) — same coalesce
    mechanism; #147 PREVENTS / value-1 FORCES; both are a specific-construct coalesce trigger, not a count/pressure.
