...
- Use `python tools/orig/source_layout.py` when you want one address-ordered per-file skeleton that interleaves retail-backed anchor windows with the short-gap file estimates so you can start claiming source boundaries directly, or `--broad-exact-layout` when you want that same layout view to include larger exact-debug corridor windows.
